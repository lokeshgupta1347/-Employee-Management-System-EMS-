...
const LoginLanding = () => {
  ...
  const portalOptions=[
    ...
    },
    {
      ...
      description:"View your profile, tarck attendence, request time off, and access payslips.",
      icon:UserIcon
    }
  ]
    ...
    <div className='min-h-screen flex flex-col md:flex-row'>
     ...
     <div className='w-full max-w-md animate-fade-in relative z-10'>
      ...
      {portalOptions.map((portal)=>(
        ...
        <h3  className="text-lg font-slate-800
                                group-hover:text-indigo-600 mb-1
                                transition-colors">{portal.title}</h3></div>
        <ArrowRightIcon className="w-4 h-4
                        text-slate-400 group-hover:text-indigo-600
                        group-hover:translate-x-1 transition-all
                        duration-300"></ArrowRightIcon>
        </Link>
      ))}
      ...
      <div className='mt-12 text-center md:text-left text-sm text-slate-400'>
        <p>c {new Date().getFullYear()} Lokesh. All rights reserved. </p>
      </div>
