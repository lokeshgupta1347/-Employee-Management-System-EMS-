        ...
        if(leaveApplication?.status==="PENDING"){
            ...
            await sendEmail({
                ...
                body:`<div style="max-width: 600px;">
                        <h2>Hi Admin, 👋</h2>
                        <p>You have a leave application in ${employee.department} today:</p>
                        <p style="font-size: 18px; font-weight: bold; color: #007bff; margin: 0px 0;">${leaveApplication?.startDate?.toLocaleDateString()}</p>
...
const attendanceReminderCron = inngest.createFunction(
    { id: "attendance-reminder-cron" ,triggers:[{cron: "0 30 5 * * ?"}]
    ...
    async ({ step }) => {
        ...
        const checkedInIds = await step.run("get-checked-in-ids", async () => {
            const memberships = await Attendance.find({
                date: {
                    $gte: new Date(today.start),
                    $lt: new Date(today.end)
                }
            }).lean();
            return memberships.map((a) => a.employeeId.toString());
        });
